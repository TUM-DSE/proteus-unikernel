...
async def remove_file_by_rm(filepath):
    remove_cmd = f"rm {filepath}"
    remove_process = await asyncio.create_subprocess_shell(remove_cmd)
...
async def main():
    ...
    for dst_fpga in dst_fpgas:
        ...
        for row in enumerate(signal_size_list):
            ...
            # prepare exec command, FPGA stays src_fpga for VM save
            exec_cmd = exec_cmd_base.copy()
            exec_cmd.append(f"fir.xclbin {signal_size}")
            # print(exec_cmd)

            # delete the shared-memory snapshot if exists
            # FIXME: we should use a safer way to remove the shm files...
            shm_snapshot_path = f"/dev/shm/{snapshot_file}"
            if os.path.exists(shm_snapshot_path):
                # remove_task = asyncio.create_task(remove_file_by_rm(shm_snapshot_path))
                # await remove_task
                os.remove(shm_snapshot_path)
                if os.path.exists(shm_snapshot_path):
                    print(f"Warning: The snapshot {snapshot_file} still exists.")
                else:
