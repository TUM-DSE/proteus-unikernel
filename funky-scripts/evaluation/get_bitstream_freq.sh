...
get_bitstream_freq() {
  ...
  for app in "${app_list[@]}"; do
    ...
    for fpga_type in $fpga_types; do
      bs=$bs_path/$app/$fpga_type/bitstream
      if [ -f "$bs" ]; then
        output="$(xclbinutil --info --input "$bs_path"/"$app"/"$fpga_type"/bitstream \
...
get_bitstream_freq $vitis_bs_path "${VITIS_EXAMPLES_APPS[@]}"
get_bitstream_freq $rosetta_bs_path "${ROSETTA_APPS[@]}"
